...
<form id="frm_comment" ref="form" @submit.prevent="onSubmit">
    ...
    <div class="input-group">
        ...
        <textarea class="form-control" name="bo_content" ref="bo_content" v-model="bo_content"></textarea>
        <b-button variant="outline-secondary" size="sm" type="submit">등록</b-button>
        <b-button
            v-if="sltIndex !== -1"
            @click="callReCommentCancel"
        ...
        async onSubmit () {
            ...
            try{
                if (this.comment_mode == 'create' || this.comment_mode == 'reCreate')
                    res = await http.post(`/api/board/${this.bo_cd}/store`, this.getFromData('store'));
                else if (this.comment_mode == 'reEdit')
                    res = await http.post(`/api/board/${this.bo_cd}/update/${this.bo_id}`, this.getFromData('update'));
