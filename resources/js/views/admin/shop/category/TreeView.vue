<template lang="html">
    <ul v-show="this.depth == 0 ? true : this.isOpen">
        <draggable :list="cate" handle=".ca_move" @change="sortUpdate">
            <li
                v-for="(ca, idx) in cate"
                :index="idx"
                :class="{hadChild: ca.childCount}"
            >
                <div class="cate"
                    v-if="!ca.update"
                    :tabindex="idx"
                ...
                >
                    <div class="ca_name" @click="clickCate(ca)">
                        {{ca.ca_name}}
                        <span v-if="ca.childCount">[{{ subOpen ? '-' : '+' }}]</span>
                    </div>
                    ...
                    <transition name="slide-fade">
                        <div class="hd_menu" v-if="ca.showMenu">
                            ...
                            <b-button variant="info" size="sm" v-if="!ca.childCount" @click="makeSub(ca)" v-b-tooltip.hover title="하위 카테고리 생성">
...
export default {
    ...
    methods: {
        ...
        recount(){
            this.$set(this.parent, 'childCount', this.parent.subCate.length);
...
#adm_category ul { margin-top:0.6rem; }
#adm_category ul li { position:relative;  }
#adm_category ul li:before {
    position:absolute; top:15px; left:-20px;
    content:""; width:10px; height:10px; border:1px solid black; border-radius:50%
}
#adm_category ul li.hadChild > .cate > .ca_name { font-weight:bold; }
#adm_category ul li.hadChild:before { background:black; }
#adm_category ul li button { border-width:0; line-height:12px; }
#adm_category ul li .cate { cursor:pointer; }
#adm_category ul li .cate .ca_name { display:inline-block; padding:8px 0; border:2px dashed #FFF; }
#adm_category ul li .cate .hd_menu { display:inline-block; position:relative; top:-2px; }
/* #adm_category ul li .cate .hd_menu button { padding:0.2rem 0.25rem; font-size:0.56rem; line-height:0; } */
#adm_category ul li .row { padding:2.5px 0; }

#adm_category ul li .cate:focus .ca_name { border-color:#262b57; border-radius:5px; background-color:#F0F0F0; }
