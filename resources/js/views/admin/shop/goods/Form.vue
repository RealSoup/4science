                    ...
                    <template v-if="value.gd_mk_currency && value.gd_mk_currency !== 'KRW'">
                        <b-col class="type04">
                            환율:<flag-icon :code="value.gd_mk_currency"></flag-icon><span>{{value.gd_mk_currency}}</span>
                                [<span style="font-weight:900; color:#000;">{{value.gd_mk_rate}}</span>] /
...
export default {
    ...
    methods: {
        ...
        insertAtOpt() { this.value.goods_option.push({ go_id:0, go_name:'', goods_option_child:[{ goc_id:0, goc_name:'', goc_dc:0, goc_price:0 }] }); },
        removeAtOpt(i) { 
        ...
        insertAtOptItem(goc) { goc.push({ goc_id:0, goc_name:'', goc_dc:0, goc_price:0 }); },
