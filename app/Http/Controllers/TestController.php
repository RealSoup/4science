...
class TestController extends Controller {
    public function search_test (Request $req) {
        ...
        $data['categorys'] = Category::getSelectedCate(
            $req->filled('ca01') ? $req->ca01 : 0,
        ...
        if ($req->filled('keyword')) {
            ...
            if (if_not_my_ip($req->ip()))
                event(new \App\Events\GoodsSearch(
                    $req->keyword,
                    auth()->check() ? auth()->user()->id : 0,
                    $req->ip(),
                    $req->filled('referer') ? $req->referer : ''
                ));
            ...
            if ($req->filled('mode')) {
                    ...
                    'maker' => [
                        'bool' => [
                            'should' => [
                                ...
                                ['prefix'   => ['mk_name.keyword' => $keyword]],
                ...
                // korean_exact로 토크나이징 → 토큰 수 확인
                $tokenResponse = $client->indices()->analyze([
                    'index' => 'shop_goods',
                    'body'  => [
                        'analyzer' => 'korean_exact',
                        'text'     => $keyword,
                    ],
                ]);
                $tokenCount = count($tokenResponse->asArray()['tokens']);
                ...
                $shouldClauses = [
                    ...
                    [
                        'multi_match' => [
                            'query'          => $keyword,
                            'fields'         => ['gd_name^3', 'gm_name_all', 'gm_name_all.exact^2', 'mk_name^2'],
                            'fuzziness'      => 'AUTO',
                            'prefix_length'  => 2,
                            'max_expansions' => 50,
                            'boost'          => 0.5,
                ...
                if ($tokenCount >= 2) {
                    // 토큰 2개 이상: AND 조건을 must로 강제 → 엉뚱한 상품 차단
                    $boolClause['must'] = [
                        [
                            'bool' => [
                                'should' => [
                                    ...
                                    ['match' => ['gd_keyword'  => ['query' => $keyword, 'analyzer' => 'korean_exact', 'operator' => 'and']]],
                                ],
                                'minimum_should_match' => 1,
                            ],
                        ],
                ...
                $searchQuery = [
                    'function_score' => [
                        ...
                        'functions' => array_merge([
                            ...
                            ['filter' => ['term'     => ['gm_catno_all.keyword' => $keyword]], 'weight' => 100000],
                            ['field_value_factor' => [
                                'field'    => 'purchase_score',
                                'factor'   => $isCatnoPattern ? 0 : 100,
                                'modifier' => 'none',
                                'missing'  => 0,
                            ]],
                        ], $personalizeFunctions),
    ...
    public function search_test__2 (Request $req) {
